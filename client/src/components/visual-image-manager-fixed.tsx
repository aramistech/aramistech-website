...
export default function VisualImageManager() {
  ...
  const replaceImageMutation = useMutation({
    mutationFn: async ({ imageId, newUrl }: { imageId: string; newUrl: string }) => {
      const response = await apiRequest(`/api/admin/replace-image`, "PUT", {
        imageId,
        newUrl,
      });
      return response;
    ...
    onSuccess: () => {
      ...
      const imageIndex = websiteImages.findIndex(img => img.id === selectedImage?.id);
      if (imageIndex !== -1 && selectedImage) {
        websiteImages[imageIndex].currentUrl = `/api/media/${selectedImage}/file`;
      }
    },
    onError: (error: any) => {
      toast({
